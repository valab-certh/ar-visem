...
shaders.vertex = `
  ...
  void main()
  {
    ...
    // mat4 vPosition =  tPos * rXPos * rZPos * rYPos * sPos;

    vUv = uv;
    vec4 mvPosition = modelViewMatrix * vec4( position , 1.0);
    gl_Position = projectionMatrix * mvPosition;

    worldCoord = modelMatrix * vPosition * vec4( position, 1.0 );
  }
...
shaders.fragmentMultipleInterpolation = `
  ...
  // space step
  uniform float xspaceStep;
  uniform float yspaceStep;
  uniform float zspaceStep;
  ...
  */
  ...
  vec4 getIntensityWorldNearest(vec3 swc){
    ...
    // to be kept
    float indexSliceToDisplay = floor(swc.z/zspaceStep + rounder);
    ...
    if(mod(swc.x/xspaceStep, 0.5) == 0.0){
      swc.x -= rounder*xspaceStep;
    }
    
    // avoid being exactely between 2 slices because it can produce unpredictable result (stripes)
    if(mod(swc.y/yspaceStep, 0.5) == 0.0){
      swc.y -= rounder*yspaceStep;
    }
