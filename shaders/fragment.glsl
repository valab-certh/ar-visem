...
// space length
uniform vec3 spaceLength;

// space step
uniform vec3 spaceStep;

uniform float forcedAlpha;
uniform mat4 containerMat;


// a texture will contain a certain number of slices
uniform sampler2D textures[maxNbOfTextures];

// perform a trilinear interpolation if true, NN if false
...
/*
// workaround for the missing texelFetch in webGL1
vec4 texelFetch( sampler2D tex, ivec2 size, ivec2 coord){
vec2 unitPos = vec2(
    float(coord.x) / float(size.x) + 1.0 / (float(size.x) * 2.0),
    float(coord.y) / float(size.y) + 1.0 / (float(size.y) * 2.0)
    );

return texture2D( tex, unitPos );
}
*/
...
// to be kept
float indexSliceToDisplay = floor(swc.z/spaceStep.z + rounder);
...
float halpPixH = sliceWidth / spaceLength.x / 2.0; 
float halpPixV = sliceHeight / spaceLength.y / 2.0;

// avoid being exactely between 2 slices because it can produce unpredictable result (stripes)
if(mod(swc.x/spaceStep.x, 0.5) == 0.0){
    swc.x -= rounder*spaceStep.x;
}

// avoid being exactely between 2 slices because it can produce unpredictable result (stripes)
if(mod(swc.y/spaceStep.y, 0.5) == 0.0){
    swc.y -= rounder*spaceStep.y;
}

// the actual textel 2D position of this 3D word coordinate
vec2 posInTexture = vec2(
    sliceWidth * colTexture + ( swc.x/spaceLength.x * sliceWidth) + halpPixH,
    sliceHeight * rowTexture + (1.0 / nbSlicePerCol  - swc.y/spaceLength.y * sliceHeight) + halpPixV
);
...
return  swc.x < 0.0 || swc.x > spaceLength.x ||
        swc.y < 0.0 || swc.y > spaceLength.y ||
        swc.z < 0.0 || swc.z > spaceLength.z;
}
...
return  swc.x < edgeSize || swc.x > (spaceLength.x - edgeSize) ||
        swc.y < edgeSize || swc.y > (spaceLength.y - edgeSize) ||
        swc.z < edgeSize || swc.z > (spaceLength.z - edgeSize);
}
...
// vWorldCoord is [0, n], but the box is centered on 0 to make rotation work better
// so vWorldCoordShifted is like vWorldCoord but shifted of half size in each direction
vec3 vWorldCoordTrasformed = vec3(containerMat * vWorldCoord);
vec3 vWorldCoordShifted = vec3(vWorldCoordTrasformed) + spaceLength/vec3(2);

// hide the outside
if( isOusideTheVolume(vWorldCoordShifted) )
{
    discard;
    return;
}
...
if( isInternalEdge(vWorldCoordShifted) )
{
...
if(trilinearInterpol){
    color = getIntensityWorldTrilinear(vWorldCoordShifted);
}else{
    color = getIntensityWorldNearest(vWorldCoordShifted);
}
